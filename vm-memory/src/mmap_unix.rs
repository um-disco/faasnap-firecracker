...
impl MmapRegion {
    ...
    /// Wrap an existing mapping described by `addr` and `size`.
    ///
    /// # Safety
    ///
    /// Caller must ensure that the provided range is a valid mapping owned by the current
    /// process. The returned [`MmapRegion`] will call `munmap` on drop, so the caller must not
    /// manage the lifetime of the mapping elsewhere.
    ...
    /// Wrap an existing anonymous mapping at `addr` with `size` bytes.
    ///
    /// This is a convenience wrapper around `from_raw_parts` for anonymous mappings
    /// (no file backing), which is commonly used with pseudo_mm.
    ///
    /// # Safety
    ///
    /// Caller must ensure that the provided range is a valid mapping owned by the current
    ...
    pub unsafe fn from_raw_ptr(
        ...
        Self::from_raw_parts(addr, size, prot, libc::MAP_PRIVATE | libc::MAP_ANONYMOUS, None)
    }
